(function ($) {
	...
	var controller = {
		...
		setSelected: function (id, clicked) {
			...
			$('#' + clicked.imageId).html('<img src="' + model.getSelected(id).localSrc + '" class="img-fluid">');
			return true;
		},
	...
	var view = {
		...
		localInit: function (type = 'local') {
			...
			controller.getAllFiles(type).forEach(function (file) {
				...
				raw += '<div class="temp-img" id="myProgress" style="background:url(' + file.localSrc + ') no-repeat;">';
				raw += '</div>';
				raw += "</div>";
			});
